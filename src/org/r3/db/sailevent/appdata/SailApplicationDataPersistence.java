...
public class SailApplicationDataPersistence extends SailApplicationDataPersistenceBase
{
					...
					PersistenceConfigInfo persistenceConfigInfo)
	{
		...
		filter.or().andEventidEqualTo(requestSiteId).andApplicationidEqualTo(applicationId).andSectionidEqualTo(aSectionId);
		
		return this.delete(filter, persistenceConfigInfo);
	}

	public SailApplicationDataDataTableAdapter queryByEventidApplicationid(String requestSiteId, String applicationId, PersistenceConfigInfo persistenceConfigInfo)
	{
		SailApplicationDataFilter filter = new SailApplicationDataFilter();
		...
		return this.query(filter, persistenceConfigInfo, queryForm.getsEcho(), queryForm.getiDisplayStart(), queryForm.getiDisplayLength(), queryForm.isExecuteCount());
