...
public class MongoDBStorageUrl extends StorageUrl
{
	...
	public MongoDBStorageUrl(String aSite, String aLanguage, String aDbname, String aCollection, String anOidAsString)
	{
		site = aSite;
		language = aLanguage;
		dbname = aDbname;
	...
	public MongoDBStorageUrl(String aUrl)
	{
		if (aUrl.startsWith("mfs://"))
		{
			// Tolgo uno slash in piu' in quanto se no lo split mi torna un
			// elemento spurio vuoto...
	...
	public static MongoDBStorageUrl newInstance(String aUrl)
	{
		try
		{
			MongoDBStorageUrl fu = new MongoDBStorageUrl(aUrl);
			if (fu.oid != null)
				return fu;
		}
	...
	{
		String s = String.format(MongoDbSchemaInfo.MongoDbFileStoreUrlPattern, site, language, MongoDbSchemaInfo.MongoDbName,
				MongoDbSchemaInfo.MongoDbFileStoreCollectionName, oid);
		return s;
	}
	...
	public Query<FileCollectionDTO> queryFilter(Datastore ds)
	{
		...
		q.or(q.criteria("_id").equal(MongoUtils.getObjectIdFromString(oid)), q.criteria("masterfile").equal(oid));
