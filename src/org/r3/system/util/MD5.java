	...
	private void update (MD5State state, byte buffer[], int offset, int length) {
		...
		if (length >= partlen) {
			...
			for (i = partlen; (i + 63) < length; i+= 64){
				// MARIO - 20160218 - Don't understand here since it seems that offset is neglected... which is fine if offset is zero...
				// It happens with offset != 0 and length > 64.. That's because in this condition it gets into the loop.
				// The fix seems to be adding offset to the invokation
				transform(state, decode(buffer, 64, i + offset));
			}
			index = 0;
		}
