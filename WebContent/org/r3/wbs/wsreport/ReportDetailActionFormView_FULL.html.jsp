...
{%  
 ...
 {%  if (!isPrinted)  {  %} 
<div class="action-bar clearfix unprintable" style="border-top: 0px; margin-bottom: 0px; margin-top: 0px">                  
...
<div class='cmsCard'>
{%  
   var theWbses = o.wbses;
   for(var i = 0; i < theWbses.getNumberOfItems(); i++)
   {
       var theWbs = theWbses.getItem(i);
...
{% if (theWbs.isExecutiveOrCoach())   {  %}
   ...
   <div class="wbs_info">
 {%=org.r3.jsportlet.wbs.dashboard.message.numberOfCompleted%}: {%=wbsStatInfo.numberOfCompleted  || '0'%},
				 {%=org.r3.jsportlet.wbs.dashboard.message.numberOfStarted%}: {%=wbsStatInfo.numberOfStarted  || '0' %},
				 {%=org.r3.jsportlet.wbs.dashboard.message.numberOfNotStarted%}: {%=wbsStatInfo.numberOfNotStarted  || '0'%},
   </div>
   <div class="wbs_info">				 
				 {%=org.r3.jsportlet.wbs.dashboard.message.numberOfOnTimeCompleted%}: {%=wbsStatInfo.numberOfCompleted - wbsStatInfo.numberOfLateCompleted  || '0'%},
				 {%=org.r3.jsportlet.wbs.dashboard.message.numberOfLateCompleted%}: {%=wbsStatInfo.numberOfLateCompleted  || '0'%}
   </div>
   <div class="wbs_info">					 
				 {%=org.r3.jsportlet.wbs.dashboard.message.numberOfOnTimeIncomplete%}: {%=(wbsStatInfo.numberOfStarted + wbsStatInfo.numberOfNotStarted - wbsStatInfo.numberOfLateIncomplete)  || '0'%},
				 {%=org.r3.jsportlet.wbs.dashboard.message.numberOfLateIncomplete%}: {%=wbsStatInfo.numberOfLateIncomplete  || '0'%}				 
   ...
   <div class="col-md-3">	
   {% if (theWbs.getROI()) { %}
   <div style='color: #777777; font-size: 1.2em; padding: 5px'>ROI: {%=theWbs.getROI()%}</div>
   {% } %}
   </div>
   </div>
...
{% 		 } 
         ...
         <h{%=(wbsNode.getNodeHSection())%}>{%=wbsNode.getNodeLabel(true)%}</h{%=(wbsNode.getNodeHSection())%}></h{%=(wbsNode.getNodeHSection())%}>
