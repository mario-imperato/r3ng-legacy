...
//       	    }
	...
	if (!includeOnlyFieldList || includeOnlyFieldList.indexOf(columnId) >= 0)
	{
    	defaultColumnDefinitions[columnId] =
    	{
    	  	"aTargets": [ -1 ],
       		"sClass": "text-left",
       		"sTitle": org.r3.db.sailevent.rankinginstancedetailview.message.boatinfo,
       		"render": function ( data, type, row, meta )
       		{
          		var item = data;
          		var s = '<b>' + (item.dataBean.sailnumber || '') + '</b> ' +
          		        '<b>' + (item.dataBean.yachtname || '') + '</b> - ' +
          		        (item.dataBean.model || '') + '<br />' +
